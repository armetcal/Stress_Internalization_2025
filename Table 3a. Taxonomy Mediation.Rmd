---
title: "3. Maaslin2"
author: "Avril Metcalfe-Roach"
date: "`r Sys.Date()`"
output: html_document
---

```{r}
library(tidyverse)
library(phyloseq)
library(ggpubr)
library(lavaan)
library(manymome)
library(haven)
library(Maaslin2)

meta = read_sav('../Data/CHILD_CBCL_MatDistress_Microbiome_09042024_ForAvril.sav')
ps = readRDS('../CHILD_phyloseq_Feb25.rds')

# Fix metadata
meta$csec[meta$csec=='NA'] = NA
meta$csec[meta$csec==''] = NA
table(is.na(meta$csec))
table(meta$csec)

meta$Mother_abs_birth_yn[meta$Mother_abs_birth_yn=='NA'] = NA
meta$Mother_abs_birth_yn[meta$Mother_abs_birth_yn==''] = NA
table(is.na(meta$Mother_abs_birth_yn))
table(meta$Mother_abs_birth_yn)

# Fix breast milk variables
meta$bf_3m = sapply(meta$BF_3m_status_4, function(x){
  return(ifelse(is.na(x),NA,ifelse(x=='',NA,
                                   ifelse(x=='Never breastfed','No', ifelse(x=='Zero','Prev','Yes')))))
})
meta$bf_3m = factor(meta$bf_3m, levels = c('No','Prev','Yes'))
meta$bf_3m_bool = meta$bf_3m
meta$bf_3m_bool[meta$bf_3m_bool=='Prev'] = 'No'
meta$BF_12m[meta$BF_12m=='' | meta$BF_12m=='NA'] = NA
meta$BF_dur = sapply(meta$BF_dur_cat2, function(x){
  return(ifelse(is.na(x),NA,ifelse(x=='Never breastfed',0,
                                   ifelse(x=='>0-3',1, ifelse(x=='3-9',2,ifelse(x=='9-12',3,4))))))
})

psmeta = prune_taxa(taxa_names(ps)[1],ps) %>% psmelt()

# Load tax-glom data, add in formatted sample info
p = readRDS('../Data/tax_glom/phylum.rds')
g = readRDS('../Data/tax_glom/genus.rds')

p@sam_data$depth = scale(sample_sums(p))
g@sam_data$depth = scale(sample_sums(g))
```

```{r Load significant taxa}
da_results = readxl::read_xlsx('Figure 2. Differential Abundance - Taxonomy.xlsx') %>% 
  filter(sig==1, metadata == Predictor)
```

# Mediation

```{r}
stats.med = tibble()

pmelt = p %>% subset_samples(Visit=='1year') %>% 
  subset_taxa(Phylum %in% da_results$taxon[da_results$Level=='Phylum']) %>% 
  psmelt() %>%
  select(Participant_ID,Dyad_ID,Sample,Abundance,Phylum,depth) %>% 
  right_join(meta) %>% droplevels() %>% 
  mutate(ZAbundance = scale(Abundance))

gmelt = g %>% subset_samples(Visit=='1year') %>% 
  subset_taxa(Genus %in% da_results$taxon[da_results$Level=='Genus']) %>% 
  psmelt() %>%
  select(Participant_ID,Dyad_ID,Sample,Abundance,Phylum,Genus,depth) %>% 
  right_join(meta) %>% droplevels() %>% 
  group_by(Genus) %>% 
  mutate(ZAbundance = scale(Abundance)) %>% ungroup
```


```{r}
Model <-'
ZInternalizingPercentile ~ ZAbundance 
ZAbundance ~ ZPSS_36weeks
'

set.seed(421)
Modelrun <- sem(Model, data=pmelt, fixed.x=TRUE, missing="ml")
summary(Modelrun, standardized=TRUE,fit.measures=TRUE, rsquare=TRUE)

# Calculate Indirect Effect 

ModelIE <- indirect_effect(x = "ZPSS_36weeks",
                           y = "ZInternalizingPercentile",
                           m = "ZAbundance",
                           fit = Modelrun,
                           boot_ci = TRUE,
                           R = 500,
                           seed = 98171,
                           ncores = 1)

modelout = print(ModelIE,pvalue=TRUE)

stats.med = rbind(stats.med,
                  tibble(Level='Phylum',Time = '1 year',Type='Univariable',Taxon='p__Proteobacteria',
                   Estimate = modelout$indirect,
                   Estimate_low = modelout$boot_ci[1],
                   Estimate_high = modelout$boot_ci[2],
                   Pval = modelout$boot_p,
                   SE = modelout$boot_se)
)

#~~~~~~~~~~~~~~~~~~~~~~~~~

Model <- '
  # Mediation paths
  ZInternalizingPercentile ~ ZAbundance
  ZAbundance ~ ZPSS_36weeks

  # Direct paths for the predictors
  ZAbundance ~ Mother_abs_birth_yn + csec + BF_dur + depth
'

set.seed(421)
Modelrun <- sem(Model, data=pmelt, fixed.x=TRUE, missing="ml")
summary(Modelrun, standardized=TRUE,fit.measures=TRUE, rsquare=TRUE)


ModelIE <- indirect_effect(x = "ZPSS_36weeks",
                           y = "ZInternalizingPercentile",
                           m = "ZAbundance",
                           fit = Modelrun,
                           boot_ci = TRUE,
                           R = 500,
                           seed = 98171,
                           ncores = 1)

modelout = print(ModelIE,pvalue=TRUE)

stats.med = rbind(stats.med,
                  tibble(Level='Phylum',Time = '1 year',Type='Multivariable',Taxon='p__Proteobacteria',
                   Estimate = modelout$indirect,
                   Estimate_low = modelout$boot_ci[1],
                   Estimate_high = modelout$boot_ci[2],
                   Pval = modelout$boot_p,
                   SE = modelout$boot_se)
)
```

```{r Genus}
to_test = unique(gmelt$Genus)[!is.na(unique(gmelt$Genus))]
for(x in to_test){
  # Univariate
  Model <-'
  ZInternalizingPercentile ~ ZAbundance 
  ZAbundance ~ ZPSS_36weeks
  '
  
  set.seed(421)
  Modelrun <- sem(Model, data= gmelt %>% filter(Genus==x), fixed.x=TRUE, missing="ml")
  summary(Modelrun, standardized=TRUE,fit.measures=TRUE, rsquare=TRUE)
  
  
  ModelIE <- indirect_effect(x = "ZPSS_36weeks",
                             y = "ZInternalizingPercentile",
                             m = "ZAbundance",
                             fit = Modelrun,
                             boot_ci = TRUE,
                             R = 500,
                             ncores=1,
                             seed=421)
  
  modelout = print(ModelIE,pvalue=TRUE)
  
  stats.med = rbind(stats.med,
                    tibble(Level='Genus',Time = '1 year',Type='Univariable',Taxon=x,
                     Estimate = modelout$indirect,
                     Estimate_low = modelout$boot_ci[1],
                     Estimate_high = modelout$boot_ci[2],
                     Pval = modelout$boot_p,
                     SE = modelout$boot_se)
  )
  
  # Multivariate
  Model <- '
  # Mediation paths
  ZInternalizingPercentile ~ ZAbundance
  ZAbundance ~ ZPSS_36weeks

  # Direct paths for the predictors
  ZAbundance ~ Mother_abs_birth_yn + csec + BF_dur + depth
  '
  
  set.seed(421)
  Modelrun <- sem(Model, data=gmelt %>% filter(Genus==x), fixed.x=TRUE, missing="ml")
  summary(Modelrun, standardized=TRUE,fit.measures=TRUE, rsquare=TRUE)
  
  
  ModelIE <- indirect_effect(x = "ZPSS_36weeks",
                             y = "ZInternalizingPercentile",
                             m = "ZAbundance",
                             fit = Modelrun,
                             boot_ci = TRUE,
                             R = 500,
                             ncores=1,
                             seed=421)
  
  modelout = print(ModelIE,pvalue=TRUE)
  
  stats.med = rbind(stats.med,
                    tibble(Level='Genus',Time = '1 year',Type='Multivariable',Taxon=x,
                     Estimate = modelout$indirect,
                     Estimate_low = modelout$boot_ci[1],
                     Estimate_high = modelout$boot_ci[2],
                     Pval = modelout$boot_p,
                     SE = modelout$boot_se)
  )
}

stats.med = stats.med %>% group_by(Level,Type) %>% 
  mutate(Padj = p.adjust(Pval,method='BH')) %>% 
  ungroup()
```

```{r Save stats}
writexl::write_xlsx(list(Mediation=stats.med),'Table 3a. Taxonomy Mediation.xlsx')
```

